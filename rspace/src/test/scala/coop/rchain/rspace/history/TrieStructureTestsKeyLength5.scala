...
class TrieStructureTestsKeyLength5
    extends HistoryTestsBase[Id, Txn[ByteBuffer], TestKey5, ByteVector]
  ...
  it should "insert skip node " in withTestTrieStore { implicit txnal => store => branch =>
  ...
  it should "build two levels of skip nodes" in withTestTrieStore {
  ...
  it should "collapse structure after delete" in withTestTrieStore {
    implicit txnal => store => branch =>
      val k1: TestKey5 = "01000"
      val k2: TestKey5 = "01100"
      insert(store, branch, k1, TestData.val1)
